...
template <bool Local> struct Hooks
{
    ...
    static HRESULT CreateForHwnd(IDXGIFactory2* factory, IUnknown* device, HWND window, const DXGI_SWAP_CHAIN_DESC1* desc,
                          ...
                          IDXGISwapChain1** swapchain, bool& skip)
    {
        ...
        if (SUCCEEDED(result) && skip && swapchain) RegisterQueue(result, skip, device, *swapchain);
        return result;
    }
    ...
    static void* Wrap(const char* name, GetFunction getFunction)
    {
        ...
        if (createHwnd) { originalCreateHwnd = reinterpret_cast<CreateHwnd>(function); return &CreateForHwnd; }
